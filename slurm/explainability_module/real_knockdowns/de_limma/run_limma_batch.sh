...
# Código R
R_DE_LIMMA_DIR="${REPO_ROOT}/src/deeprbp/explainability_module/real_knockdowns/de_limma"
...
run_limma() {
  ...
  Rscript "${R_DE_LIMMA_DIR}/run_voom-limma.R" \
...
Rscript "${R_DE_LIMMA_DIR}/just_concatenate_plots.R" \
  --base_dir "${OUT_ROOT}" \
  --logfc_thresh "${LOGFC_THRESH}" \
  --p_cut_type "${P_CUT_TYPE}" \
  --p_cut_value "${P_CUT_VALUE}"
