...
class CustomDataset(Dataset):
    ...
    def __init__(self, 
                 ...
                 select_tumor_types: Optional[List[str]] = None,
        ...
        along with metadata to perform sample selection, split data if needed and scaling
        Args:
            config (dict): Configuration dictionary.
            #source_name (str, optional): Dataset name in config to load paths from, e.g., 'TCGA' or 'GTEX'.
            paths (dict, optional): Dictionary with paths to the data files. If not provided, it uses the config paths for source_name.
            select_tumor_types (list, optional): List of tumor types to select samples from. Can be 'all' or a list of strings. If None, selection will be skipped.
        ...
        self.config = config
        self.paths = paths
        self.select_tumor_types = select_tumor_types
        ...
        # Determine source_name based on the configuration of training or validation
        self.source_name = self._get_source_name()
        # Load and process data
        self.data = self._load_data()
        self._check_consistency()
        # Select samples and perform splits if indices are not provided
        self._process_samples_and_splits()
    ...
    def _get_source_name(self):
        """ Determine the source name based on the configuration. """
        if 'train_prediction_model' in self.config:
            return self.config['train_prediction_model']['source_name']
        else:
            return self.config['val_prediction_model']['source_name']
    ...
    def _load_data(self):
        ...
        if self.paths is None:
            paths = self.config['data']['training_module'][self.source_name]
        else:
            paths = self.paths
        try:
            return {
    ...
    # Procesamiento de muestras y divisiones:
    def _process_samples_and_splits(self):
        """ Select samples based on tumor types and perform data splits. """
        if self.train_idx is None and self.valid_idx is None and self.test_idx is None:
            if self.select_tumor_types is not None:
                self.data = self.filter_samples(self.data, self._select_samples())
        # Store sample IDs and perform splits
    ...
    def _select_samples(self):
        ...
        if self.select_tumor_types == 'all':
        ...
        elif isinstance(self.select_tumor_types, list):
            return self.data["metadata_df"].index[self.data["metadata_df"][self.sample_category].isin(self.select_tumor_types)]
        else:
            raise ValueError("select_tumor_types should be 'all' or a list of strings.")
    ...
    def _perform_splits(self) -> None:
        ...
        """
        # Determinar si estamos en modo de entrenamiento-validación (TCGA) o solo validación (GTEX)
        is_train = 'train_prediction_model' in self.config
        if is_train:
            # Modo de entrenamiento: - TCGA, proceder con la división (la división del data ya ha podido ser determinada anteriormente)
            ...
            # Modo de validación - No se realizan divisiones
            print("Validation mode: Skipping train/test or train/val split.")
            self.train_idx = None
            self.valid_idx = None
            self.test_idx = [self.id2index_mapping[patient_id] for patient_id in self.sample_ids]  # Asignar todas las muestras a test_idx
            self.add_sample_set_label(set_name='testing', samples=self.index2id(self.id2index_mapping, self.test_idx))
    ...
    def _perform_scaling(self):
        ...
        # Inicializar selected_train_sample_ids
        selected_train_sample_ids = None
        ...
        # If the scaler is not set and we are in training mode, we need to fit it
        if self.scaler is None and self.sigma is None and 'train_prediction_model' in self.config:
        ...
        self.transform_data(selected_train_sample_ids, 'train')
        ...
        self.transform_data(selected_test_sample_ids, 'test')
        ...
        if self.valid_idx is not None:
            ...
            if len(selected_valid_sample_ids) == 0:
                ...
                self.transform_data(selected_valid_sample_ids, 'validation')
    ...
    def transform_data(self, sample_ids, data_type):
        """Transform the data for the given sample IDs."""
        if sample_ids is not None and len(sample_ids) > 0:
            self.data['rbp_expr_df'].loc[sample_ids] = self.scale_rbp_expression(
                transform_set=self.data['rbp_expr_df'].loc[sample_ids], 
                scaler=self.scaler, 
                sigma=self.sigma
            )
        else:
            print(f"[transform_data] Warning: No samples available for {data_type} data transformation.")
