...
def main():
    ...
    config_parser = ConfigParser(args.config_path_file)
...
def objective(trial, config, train_dataset, valid_dataset, val_batch_size, getBM, output_dir):
    ...
    print(f"    - Hidden Layer 1 Nodes: {config['hidden1_nodes']}")
    print(f"    - Use Uniform Nodes: {config['uniform_nodes']}")
    print(f"    - Node Shrink Factor: {config['node_shrink_factor']}")
    print(f"    - Activation Function: {config['activation_func']}")
    ...
    try:
        ...
        print("\n[*] Model Summary:")
        summarize_model(model, train_loader, config["batch_size"])
