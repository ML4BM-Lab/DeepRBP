#' Create Visualizations for POSTAR Data Using DeepLIFT Scores
#'
...
#'                        that contains the calculated scores.
#' @param list_rbps_postar_filename A character string indicating the name of the RBP list file, 
#' which is ordered in descending order by the number of positive regulations associated with that RBP in POSTAR (CSV).
#' @param list_genes_postar_filename A character string indicating the name of the gene list file,
#' which is ordered in descending order by the number of positive regulations associated with that gene in POSTAR (CSV).
...
#'                     list_rbps_postar_filename = "rbps.csv", 
#'                     list_genes_postar_filename = "genes.csv", 
...
#max_iterations <- 7
  ...
  path_getbm <- file.path(input_path, getBM_filename)
  
  df_results_summary <- data.table::fread(path_summary_results) %>% as.data.frame() 
  list_rbps_postar <- data.table::fread(path_list_rbps_postar) %>% as.data.frame()
  list_rbps_postar <- list_rbps_postar$RBPs
  list_genes_postar <- data.table::fread(path_list_genes_postar) %>% as.data.frame()
  list_genes_postar <- list_genes_postar$Genes
  ...
  #print(df_results_summary[1:5, 1:5])
  #print(list_rbps_postar[1:5])
  #print(list_genes_postar[1:5])
  
  # Drop genes that are not in Postar (10 genes)
  gene_ids_to_remove <- c('ENSG00000100311', 'ENSG00000101349', 'ENSG00000121966', 
                          'ENSG00000126249', 'ENSG00000129675', 'ENSG00000139289', 
                          'ENSG00000176105', 'ENSG00000182712', 'ENSG00000183579', 
                          'ENSG00000213999')
  
  df_results_summary <- df_results_summary %>%
    filter(!Gene_ID %in% gene_ids_to_remove)
  ...
  df_results_summary$Postar_Score[is.na(df_results_summary$Postar_Score)] <- "NA"
  df_results_summary <- df_results_summary %>% mutate(Postar_Score = factor(Postar_Score, levels = c("1","0", "NA"), labels = c("1","0","NA")))
  ...
  df_results_summary1 <- df_results_summary %>% 
    ...
    group_by(RBP_ID) %>%
    filter(all(c(0, 1) %in% Postar_Score)) %>%  # Keep only RBP_ID with both values
  ...
  df_results_summary2 <- df_results_summary %>%
    ...
    group_by(Gene_ID) %>%
    filter(sum(Postar_Score == "1") >= 5,  # Ensure at least 5 in group 1
           sum(Postar_Score == "0") >= 5) %>%  # Ensure at least 5 in group 0
    ungroup() %>%  # Ungroup to avoid issues in subsequent steps
  ...
  while (label_counter < max_iterations) {
    ...
    plotlist[[2]] <- ggboxplot(
      ...
      scale_fill_manual(values = c("#7fc97f", "#beaed4", "#fdc086"),  
                        breaks = c("1", "0", "NA"),
                        labels = c("1", "0", "NA")) +  # Labels for the legend) +
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
      scale_y_continuous(expand = c(0,0.25)) +
      stat_pvalue_manual(
