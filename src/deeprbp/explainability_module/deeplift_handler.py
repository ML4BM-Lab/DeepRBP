...
class DeepLiftHandler:
    ...
    def __init__(self, config_explain, dataset, model, verbose=1):
        ...
        cuda_requested = self._config_to_bool(self.config_explain.get("cuda", False))
        ...
        self.model.eval()  # BN en eval para atribuciones estables

        self.logger.log(
            f"✅ DeepLiftHandler initialized successfully on device: {self.device}.",
            level=1
        )

    @staticmethod
    def _config_to_bool(value):
        """
        Robust boolean parser for config values that may come as bool, str or int.
        """
    ...
    def _to_model_device(self, x):
        """
        Move tensors / numpy arrays / pandas objects to the same device as the model.

        Captum requires inputs, baselines, additional_forward_args and model
        parameters to live on the same device. This helper prevents CPU/GPU
        mismatches during DeepLIFT execution.
        """
    ...
    def prepare_rbp_reference_tensor(self):
        ...
        if reference_type == 'median_reference':
            ...
            reference_rbp_tensor = torch.tensor(
                np.median(self.dataset.features['scaled_rbp_df'], axis=0),
                dtype=torch.float32
            )
            ...
            reference_rbp_tensor = torch.zeros(
                1,
                self.dataset.features['scaled_rbp_df'].shape[1],
                dtype=torch.float32
            )
            ...
            reference_rbp_tensor = torch.ones(
                1,
                self.dataset.features['scaled_rbp_df'].shape[1],
                dtype=torch.float32
            ) * 0.5
    ...
    def compute_attribution_scores(self, reference_rbp_tensor):
        ...
        # Captum inputs, baselines, additional_forward_args and model
        # must all be on the same device.
        scaled_rbp_tensor = self._to_model_device(
            self.dataset.features['scaled_rbp_df']
        )  # (B, R)

        gene_tensor = self._to_model_device(
            self.dataset.features['gene_df']
        )  # (B, T)

        reference_rbp_tensor = self._to_model_device(
            reference_rbp_tensor
        )  # (1, R)
        ...
        if mode == "final":
            ...
            use_neuron = False
            self.logger.log(
                "[compute_attribution_scores] mode=final ⇒ DeepLift(self.model), target = transcript index in log2(tpm+1).",
                level=1
            )
        else:
            ...
            use_neuron = True
            self.logger.log(
                "[compute_attribution_scores] mode=logit ⇒ NeuronDeepLift(..., Linear(128→T)), neuron_selector = transcript index.",
                level=1
            )

        self.logger.log(
            f"[compute_attribution_scores] device check: "
            f"model={next(self.model.parameters()).device}, "
            f"rbp={scaled_rbp_tensor.device}, "
            f"gene={gene_tensor.device}, "
            f"baseline={reference_rbp_tensor.device}",
            level=1
        )
        ...
        for k in tqdm(range(T), desc="Calculating scores", unit="node"):
            if use_neuron:
                ...
                # Attribution to the final output of the model
                scores = explainer.attribute(
                    inputs=scaled_rbp_tensor,          # (B, R)
                    baselines=reference_rbp_tensor,    # (1, R)
                    target=k,                          # 0..T-1
    ...
    def calculate_scores_hidden_layer(self):
        """
        Attributions from RBPs -> neurons of the last hidden layer (after the 128-unit ReLU).
        Uses NeuronDeepLift on self.model.abundance_estimator[8] (the final ReLU).
        """
        ...
        reference_tensor = self._to_model_device(
            self.prepare_rbp_reference_tensor()
        )

        rbp_inputs = self._to_model_device(
            self.dataset.features['scaled_rbp_df']
        )  # (B, R)

        gene_inputs = self._to_model_device(
            self.dataset.features['gene_df']
        )

        self.logger.log(
            f"[calculate_scores_hidden_layer] device check: "
            f"model={next(self.model.parameters()).device}, "
            f"rbp={rbp_inputs.device}, "
            f"gene={gene_inputs.device}, "
            f"baseline={reference_tensor.device}",
            level=1
        )
        ...
        for neuron_idx in tqdm(range(hidden_dim), desc="Calculating HL scores", unit="neuron"):
            scores = neuron_explainer.attribute(
                ...
                attribute_to_neuron_input=True  # pre-ReLU
            )  # -> Tensor (B, R)
    ...
    def reduce_batch_dimension(self, list_batch_scores, row_labels, col_labels, max_zero_std_logs: int = 20):
        ...
        max_zero_std_logs : int
            Max number of (row, col) pairs with std == 0 to log.

        Returns
        -------
        pd.DataFrame
            Matrix with shape K x R.
        """
        ...
        if method == 't-statistic':
            ...
            if num_zero > 0:
                ...
                for i, (r, c) in enumerate(idxs[:max_zero_std_logs]):
                    self.logger.log(
                        f"[reduce_batch_dimension] Zero std at Row: {row_labels[int(r)]}, Col: {col_labels[int(c)]}.",
                        level=1
                    )
